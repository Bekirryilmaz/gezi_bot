...
5. **Tasarım çıtası "oha" + logo zinciri (2026-09-05):** Sitede scroll animasyonları, etkileyici hero, mikro etkileşimler ve seçilen logoya site genelinde küçük atıflar (motifler) olacak — görev T-16. Logo zinciri: kullanıcı + Hiranur seçer → Cursor seçileni vektöre çevirip üretim varlıklarını yapar (T-03) → motif dili siteye dokunur (T-16). Not: Cursor'ın ilk tasarım turu (`/tasarim-secim`, 5 SVG konsept) kullanıcı tarafından "yavan/çocuk işi" bulunup reddedildi; çıta bu kayıtla yükseltildi. **Tüm tasarım işleri Cursor'dadır (kullanıcı talimatı):** keşif turu T-16 Faz 1'de ajans çıtasıyla yapılacak (araştırma + mockup duvarı + yinelemeli doğrulama); Arena tarafı tasarım ÜRETMEZ.
