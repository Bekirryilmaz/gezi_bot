...
9. **K9 — Hero hareketi (2026-09-05; mekanizma 2026-09-06 iptal; video niyeti 2026-09-08):** Niyet güncellendi: hero plakasında **kaydırma-scrub video** (Kling AI ile üretilecek tek video) — sözleşme `plan/tasarim/yon-v3.md`. Kilitler: LCP her zaman poster kare (`<video>` LCP olamaz); reduced-motion'da video mount edilmez (statik poster, 0 istek); pin / scroll-jack / otomatik oynatma / ses / loop yok; bölge-nötr ufuk (K6), ağır denizcilik teması yok (K8); P4 slayt iptali **durur** (tek sürekli kamera, kesmeli montaj yok). **Video gelene kadar** `plan/tasarim/yon-v2.md` kanvas sözleşmesi (`HeroUfuk` prosedürel ufuk + statik kapak) yürürlüktedir; video bağlanınca kanvas kapanır, scrub `video.currentTime`'a bağlanır. Künye: `plan/tasarim/gorsel-kunye.md` (kapak karesi; Kling video künyesi teslimde eklenir).
10. **K10 — Model bütçesi (2026-09-05):** Yaratıcı yön **Opus** ile tek oturumda kapatılmıştır (çıktı: `plan/tasarim/yon.md`; hero revizyonu `yon-v2.md`); **tüm uygulama Grok 4.6 extra high** ile yapılır. **Cursor Auto modu kullanılmaz.** Yeni yaratıcı tur açmadan önce `yon.md` + `yon-v2.md` yeterli mi diye bakılır; doküman cevaplıyorsa Opus'a dönülmez.
...
| `tasarim/` | `yon.md` — görsel yön, bileşen + hareket + yerleşim; `yon-v2.md` — P4-REV hero (prosedürel ufuk, slayt yasağı) |
