...
**Yapılacaklar:** (1) `plan/logo/` içindeki seçtiğim konsept SVG'yi `site/public/logo/` altına taşı; tam renk + tek renk beyaz + tek renk koyu varyantlarını üret. (2) Favicon seti: `favicon.ico` (16/32/48), `apple-touch-icon.png` (180), PWA `icon-192.png`, `icon-512.png` + `manifest` kayıtları. (3) `globals.css` `@theme` içine `plan/04_marka_ve_tema.md` §3'teki tokenları ekle (`--color-samandira: #D6402C` vb.) — mevcut deniz/kopuk/kumsal/gunes tokenlarına DOKUNMA. (4) Header'a kompakt logo (mobil: yalnız mark), footer'a logo + §7 güven/atıf satırları. (5) Branded Türkçe `not-found.tsx` (404): şamandıra illüstrasyonu + "Sayfa bulunamadı — pusula şaştı" + Keşfet/Rota CTA'ları (şu an İngilizce Next default dönüyor). (6) Statik `og-default.png` üret (next/og ile `opengraph-image.tsx` T-07'de).
