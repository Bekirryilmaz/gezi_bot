...
FRESHNESS_POLITIKALARI: dict[str, dict[str, str | int | None]] = {
    "amac_destegi": {"kategori": "yer_turu", "gerekce": "Kaynak siniflandirmasi degisebilir.", "yeniden_dogrulama": "Yeni OSM cekimi veya tur degisikligi.", "azami_yas_gun": None},
    "yer_turu": {"kategori": "yer_turu", "gerekce": "Faaliyet veya kullanim degisebilir.", "yeniden_dogrulama": "Yeni OSM cekimi veya kimlik duzeltmesi.", "azami_yas_gun": None},
    "adres": {"kategori": "kimlik_cografya", "gerekce": "Adres yapisal ama degisebilir.", "yeniden_dogrulama": "Yeni OSM cekimi veya adres duzeltmesi.", "azami_yas_gun": None},
    "telefon": {"kategori": "iletisim", "gerekce": "Isletmeciyle hizli degisebilir.", "yeniden_dogrulama": "Yeni OSM cekimi veya isletmeci degisikligi.", "azami_yas_gun": None},
    "web_sitesi": {"kategori": "iletisim", "gerekce": "Alan adi ve isletmeci degisebilir.", "yeniden_dogrulama": "Yeni OSM cekimi veya isletmeci degisikligi.", "azami_yas_gun": None},
    "wifi": {"kategori": "olanak", "gerekce": "Hizmet sunumu degisebilir.", "yeniden_dogrulama": "Yeni OSM cekimi veya yerel dogrulama.", "azami_yas_gun": None},
    "tekerlekli_sandalye_erisimi": {"kategori": "fiziksel_erisim", "gerekce": "Tadilat veya kullanilan bolum kapsami degisebilir.", "yeniden_dogrulama": "Yeni OSM cekimi ve kritik kullanimda insan kontrolu.", "azami_yas_gun": None},
    "ucretsiz": {"kategori": "maliyet", "gerekce": "Ucret politikasi degisebilir.", "yeniden_dogrulama": "Yeni OSM cekimi veya resmi tarife kontrolu.", "azami_yas_gun": None},
}
...
def osm_politikasini_uygula(oturum: Session) -> KaynakPolitikasi:
    ...
    politika.gecerli_baslangic = datetime.now(timezone.utc)
...
def _batch_getir(oturum: Session, dosya: Path, satirlar: list[dict[str, Any]]) -> VeriBatch:
    ...
    batch = VeriBatch(
        ...
        baslama_zamani=min(zamanlar) if zamanlar else datetime.now(timezone.utc),
...
def _inceleme_ekle(oturum: Session, iddia: Iddia, surum: IddiaSurumu) -> bool:
    ...
    oturum.add(IncelemeDosyasi(
        ...
        acan_aktor_id=None,
        karar_gerekcesi="Yapilandirilmis OSM alanindan deterministik candidate; insan incelemesi bekliyor.",
    ))
    return True
...
def adaylari_yaz(
    ...
    *,
    ...
    for _, aday in tum_adaylar:
        ...
        gozlem_id = _uuid5(f"observation:{batch.id}:{aday.kaynak_kayit_id}:{aday.kaynak_alani}:{gozlem_hash}")
        ...
        if onceki and onceki.deger == {"deger": aday.deger}:
            ozet.ayni_aday += 1
            continue
        if onceki and (onceki.yayin_durumu == "geri_cekilmis" or onceki.bilgi_durumu == "geri_cekilmis"):
        ...
        oturum.add(surum)
        oturum.flush()
        oturum.add(KanitBaglantisi(iddia_surumu_id=surum.id, gozlem_id=gozlem.id, rol="supporting", gerekce=f"Yapilandirilmis kaynak alani: {aday.kaynak_alani}"))
        if onceki:
            onceki_baglar = oturum.query(KanitBaglantisi).filter_by(iddia_surumu_id=onceki.id, rol="supporting").all()
            for bag in onceki_baglar:
                oturum.add(KanitBaglantisi(iddia_surumu_id=surum.id, gozlem_id=bag.gozlem_id, rol="counter", gerekce="Onceki aktif surumle yapilandirilmis deger celiskisi."))
...
def coverage_raporu(oturum: Session, *, kategoriler: set[str]) -> dict[str, Any]:
    ...
    yayinli_claimler = (
        ...
        .filter(YayinKaydi.aktif_mi.is_(True), YayinKaydi.durum.in_(("yayinlanabilir", "sinirli_yayinlanabilir")))
        .all()
    )
...
def _ana() -> None:
    ayristirici = argparse.ArgumentParser(description="Gercek OSM verisinden guvenli Samsun pilot claim candidate'lari uretir.")
    ...
    ayristirici.add_argument("--yaz", action="store_true", help="Yalniz development DB'ye candidate ve inceleme kaydi yazar.")
    ayristirici.add_argument("--osm-politikasini-uygula", action="store_true", help="Repo ve resmi ODbL dayanagina bagli OSM hak kaydini uygular.")
    ayristirici.add_argument("--summary", action="store_true", help="Mevcut pilot coverage raporunu da yazdirir.")
    args = ayristirici.parse_args()
    if args.yaz and os.environ.get("UYGULAMA_ORTAMI", "development").lower() in {"production", "prod", "canli"}:
    ...
    secilen = pilot_satirlarini_sec(tum, kategoriler=kategoriler, ilceler=set(args.ilceler) if args.ilceler else None, kategori_basina_limit=args.kategori_basina_limit)
    with OturumUretici() as oturum:
        ...
        if args.yaz:
            oturum.commit()
        print(json.dumps({"mod": "candidate-only" if args.yaz else "dry-run", "otomatik_yayin": False, "ozet": ozet.sozluk()}, ensure_ascii=False, indent=2, default=str))
        if args.summary:
            print(json.dumps({"coverage": coverage_raporu(oturum, kategoriler=kategoriler)}, ensure_ascii=False, indent=2, default=str))
