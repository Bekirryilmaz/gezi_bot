...
def _gecici_kaynak_bagi(oturum: Session) -> tuple[Yer, Sube, str]:
    ...
    oturum.add(YerKaynak(yer_id=yer.id, kaynak="openstreetmap", kaynak_id=kaynak_id, kaynak_url=f"https://www.openstreetmap.org/{kaynak_id}", sube_id=sube.id))
    oturum.flush()
...
def test_unknown_ve_geri_cekilmis_hak_candidate_yazmaz():
    ...
    try:
        ...
        if politika is None:
            ...
            oturum.add(politika)
        politika.kamusal_gosterim = politika.turev_iddia = politika.ai_isleme = politika.uzun_sureli_saklama = "bilinmiyor"
        oturum.flush()
        ozet = adaylari_yaz(oturum, dosya=Path("test.jsonl"), satirlar=[_satir(kaynak_id)], dry_run=False)
        assert ozet.hak_nedeniyle_engellenen == 4
        politika.kamusal_gosterim = politika.turev_iddia = politika.ai_isleme = politika.uzun_sureli_saklama = "izinli"
        politika.gecerli_bitis = datetime.now(timezone.utc) - timedelta(seconds=1)
        oturum.flush()
...
def test_idempotent_rerun_duplicate_uretmez_ve_yanlis_sube_baglanmaz():
    ...
    try:
        ...
        ilk = adaylari_yaz(oturum, dosya=Path("test.jsonl"), satirlar=[_satir(kaynak_id)], dry_run=False)
        ikinci = adaylari_yaz(oturum, dosya=Path("test.jsonl"), satirlar=[_satir(kaynak_id)], dry_run=False)
        assert ilk.yeni_claim == 4 and ilk.yeni_inceleme == 4
        assert ikinci.yeni_claim == 0 and ikinci.yeni_surum == 0 and ikinci.ayni_aday == 4
        assert {str(i.sube_id) for i in oturum.query(Iddia).filter(Iddia.sube_id == sube.id).all()} == {str(sube.id)}
        once = oturum.query(Iddia).count()
        yanlis = adaylari_yaz(oturum, dosya=Path("test.jsonl"), satirlar=[_satir(f"node/{uuid.uuid4().int}")], dry_run=False)
...
def test_deger_degisiminde_supporting_ve_counter_evidence_ile_conflict():
    ...
    try:
        osm_politikasini_uygula(oturum)
        _, sube, kaynak_id = _gecici_kaynak_bagi(oturum)
        adaylari_yaz(oturum, dosya=Path("test.jsonl"), satirlar=[_satir(kaynak_id, wifi=True)], dry_run=False)
        adaylari_yaz(oturum, dosya=Path("test-2.jsonl"), satirlar=[_satir(kaynak_id, wifi=False)], dry_run=False)
        ...
        assert {rol for (rol,) in oturum.query(KanitBaglantisi.rol).filter_by(iddia_surumu_id=surum.id).all()} == {"supporting", "counter"}
    finally:
        oturum.close(); islem.rollback(); baglanti.close()
...
def test_admin_approve_publication_reject_ve_public_dto_raw_evidence_sizdirmaz():
    ...
    try:
        ...
        admin = admin_olustur(oturum, eposta=f"pilot-{uuid.uuid4().hex}@test.local", gorunen_ad="Pilot", parola="guvenli-test-parolasi", roller={"yonetici"})
        wifi = oturum.query(Iddia).filter_by(sube_id=sube.id, aile="wifi").one()
        komut = IncelemeDosyasi(dosya_turu="claim", nesne_turu="claim", nesne_id=wifi.id, durum="isleniyor", risk_sinifi="normal", onerilen_eylem="claim_approve", komut_payload={}, acan_aktor_id=admin.id)
        oturum.add(komut); oturum.flush()
        _komutu_uygula(oturum, komut, admin.id, "Yapilandirilmis kanit ve kapsam kontrol edildi", f"approve-{uuid.uuid4()}")
        ...
        red = IncelemeDosyasi(dosya_turu="claim", nesne_turu="claim", nesne_id=adres.id, durum="isleniyor", risk_sinifi="normal", onerilen_eylem="claim_reject", komut_payload={}, acan_aktor_id=admin.id)
        oturum.add(red); oturum.flush()
        _komutu_uygula(oturum, red, admin.id, "Adres kapsami insan kontrolunde reddedildi", f"reject-{uuid.uuid4()}")
        assert oturum.query(YayinKaydi).filter_by(nesne_turu="claim", nesne_id=adres.id).one().durum == "yayinlanamaz"
