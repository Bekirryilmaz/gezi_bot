...
def admin_ortami():
    ...
    ek = uuid.uuid4().hex
    yonetici = admin_olustur(
        oturum,
        eposta=f"yonetici-{ek}@test.local",
        gorunen_ad="Yonetici",
        parola="cok-guvenli-test-parolasi",
        roller={"yonetici"},
    )
    gozlemci = admin_olustur(
        oturum,
        eposta=f"gozlemci-{ek}@test.local",
        gorunen_ad="Gozlemci",
        parola="cok-guvenli-test-parolasi",
        roller={"gozlemci"},
    )
    ...
    try:
        ...
        admin_uygulama.dependency_overrides.clear()
        oturum.close()
        islem.rollback()
        baglanti.close()
...
def _login(client: TestClient, eposta: str) -> str:
    yanit = client.post(
        "/v1/admin/login", json={"eposta": eposta, "parola": "cok-guvenli-test-parolasi"}
    )
...
def test_merge_authorization_ve_idor_default_deny(admin_ortami):
    ...
    yerler = oturum.query(Yer).limit(2).all()
    assert len(yerler) == 2
    aday = EslemeAdayi(
        sol_sube_id=yerler[0].id, sag_sube_id=yerler[1].id, confidence=0.7, belirsizlik={}
    )
    oturum.add(aday)
    oturum.flush()
    csrf = _login(client, gozlemci.eposta)
    cevap = client.post(
        "/v1/admin/incelemeler",
        headers={"X-CSRF-Token": csrf},
        json={
            "eylem": "canonical_merge",
            "nesne_turu": "esleme_adayi",
            "nesne_id": aday.id,
            "gerekce": "Yetkisiz merge denenmemeli",
            "payload": {"hedef_sube_id": yerler[0].id},
        },
    )
    assert cevap.status_code == 403
    gozlemci.kapsam = [str(uuid.uuid4())]
    oturum.flush()
...
def test_kritik_islem_ayni_kisi_onaylayamaz_ikinci_inceleme_audit_uretir(admin_ortami):
    ...
    cevap = client.post(
        ...
        json={
            ...
            "nesne_turu": "yer",
            "nesne_id": yer.id,
            "gerekce": "Yanlis olumlu bilginin yayini durdurulsun",
            "payload": {"gerekce_kodu": "dogruluk"},
        },
    )
    ...
    kendi = client.post(
        f"/v1/admin/incelemeler/{dosya['id']}/ikinci-onay",
        headers={"X-CSRF-Token": csrf},
        json={"gerekce": "Kendi onayim olmamali", "beklenen_surum": dosya["surum"]},
    )
    assert kendi.status_code == 409
    client.cookies.clear()
    risk_csrf = _login(client, risk.eposta)
    onay = client.post(
        f"/v1/admin/incelemeler/{dosya['id']}/ikinci-onay",
        headers={"X-CSRF-Token": risk_csrf},
        json={"gerekce": "Bagimsiz ikinci inceleme tamamlandi", "beklenen_surum": dosya["surum"]},
    )
...
def test_audit_db_trigger_ile_update_delete_edilemez(admin_ortami):
    ...
    olay = AdminAuditOlayi(
        aktor_id=yonetici.id,
        eylem="test",
        nesne_turu="test",
        nesne_id="1",
        gerekce="Append only dogrulamasi",
        istek_id="test",
        korelasyon_id="test",
        meta={},
    )
...
def test_tekerlekli_sandalye_claimi_ikinci_inceleme_ister(admin_ortami):
    ...
    sahte_oturum = AdminOturum(
        ...
        sona_erme_zamani=datetime.now(UTC) + timedelta(hours=1),
    )
    baglam = AdminBaglami(
        yonetici,
        sahte_oturum,
...
def test_claim_kuyrugu_aile_durum_mekan_ve_sayfalama_sunar(admin_ortami):
    ...
    assert all(
        kayit["aile"] == "wifi" and kayit["durum"] == "bekliyor" for kayit in govde["kayitlar"]
    )
    assert all(
        kayit["mekan_adi"] and kayit["kaynak_alani"] == "ozellikler.wifi"
        for kayit in govde["kayitlar"]
    )
...
def test_admin_dahili_sinyal_ve_birinci_el_gozlem_public_yayinlamaz(admin_ortami):
    ...
    surum = (
        oturum.query(IddiaSurumu).filter_by(iddia_id=iddia.id, surum_no=iddia.aktif_surum_no).one()
    )
    assert surum.yayin_durumu == "inceleme_bekliyor"
    assert (
        "/v1/admin/dahili-sinyaller"
        not in __import__("sunucu.api.uygulama", fromlist=["uygulama"]).uygulama.openapi()["paths"]
    )
    client.post("/v1/admin/logout", headers={"X-CSRF-Token": csrf})


def test_grup_inceleme_api_gozlemciye_kapali_pilot_okunur(admin_ortami):
    client, _, _yonetici, gozlemci, _ = admin_ortami
    csrf = _login(client, gozlemci.eposta)
    gruplar = client.get("/v1/admin/kuyruk/gruplar")
    assert gruplar.status_code == 200, gruplar.text
    assert gruplar.json()["otomatik_yayin"] is False
    yasak = client.post(
        "/v1/admin/grup-inceleme",
        headers={"X-CSRF-Token": csrf},
        json={
            "dosya_idleri": ["00000000-0000-0000-0000-000000000001"],
            "gerekce": "Yetkisiz grup inceleme denenmemeli",
        },
    )
    assert yasak.status_code == 403
    pilot = client.get("/v1/admin/pilot")
    assert pilot.status_code == 200, pilot.text
    govde = pilot.json()
    assert govde["havuz"]["sayi"] >= 1
    assert "rota_hazir" in govde["rota_durum"]
    assert govde["gold_sayisi"] <= 40
    client.post("/v1/admin/logout", headers={"X-CSRF-Token": csrf})
