...
function V(e) {
  let t = typeof e;
  if (t === `number` || t === `boolean` || t === `string` || e == null)
  ...
  let n = Object.keys(e).sort(),
    r = `{`;
  for (let t = 0; t < n.length; t++) r += `${JSON.stringify(n[t])}:${V(e[n[t]])},`;
  return `${r}}`;
}
function H(e) {
  let t = ``;
  for (let n of z) t += `/${V(e[n])}`;
  return t;
}
function U(e, t) {
  let n = {};
  for (let r = 0; r < e.length; r++) {
    let i = (t && t[e[r].id]) || H(e[r]);
    t && (t[e[r].id] = i);
    let a = n[i];
    ((a ||= n[i] = []), a.push(e[r]));
  }
...
var W = class {
    ...
    update(e, t, n) {
      for (let t of e) {
        ...
        let e = (this._layers[t.id] = h(t, n));
        ((e._featureFilter = ie(e.filter, `layers[${t.id}].filter`, n)),
          this.keyCache[t.id] && delete this.keyCache[t.id]);
      }
      for (let e of t)
        (delete this.keyCache[e], delete this._layerConfigs[e], delete this._layers[e]);
      this.familiesBySource = {};
      let r = U(Object.values(this._layerConfigs), this.keyCache);
      for (let e of r) {
  ...
  G = class {
    constructor(e) {
      ...
      let { w: r, h: i } = c(n),
        a = new d({ width: r || 1, height: i || 1 });
      for (let n in e) {
        let r = e[n];
        for (let e in r) {
          let i = r[e];
          if (!i || i.bitmap.width === 0 || i.bitmap.height === 0) continue;
          let o = t[n][e].rect;
          d.copy(i.bitmap, a, { x: 0, y: 0 }, { x: o.x + 1, y: o.y + 1 }, i.bitmap);
        }
      }
...
var K = class {
  ...
  async parse(e, t, n, i, a) {
    ((this.data = e), (this.collisionBoxArray = new _()));
    let o = new S(Object.keys(e.layers).sort()),
      s = new x(this.tileID, this.promoteId);
    s.bucketLayerIDs = [];
    let c = {},
      l = {
        featureIndex: s,
    ...
    for (let t in u) {
      ...
      for (let e = 0; e < r.length; e++) {
        let n = r.feature(e),
          o = s.getId(n, t);
        a.push({ feature: n, id: o, index: e, sourceLayerIndex: i });
      }
      for (let e of u[t]) {
        let t = e[0];
        (t.source !== this.source &&
          m(`layer.source = ${t.source} does not equal this.source = ${this.source}`),
          !t.isHidden(this.zoom, !0) &&
            (q(e, this.zoom, n),
            (c[t.id] = t.createBucket({
            ...
            })).populate(a, l, this.tileID.canonical),
            s.bucketLayerIDs.push(e.map((e) => e.id))));
      }
    }
    let d = b(l.glyphDependencies, (e) => Object.keys(e));
    ...
    if (Object.keys(d).length) {
      ...
      (this.inFlightDependencies.push(e),
        (p = i.sendAsync(
          {
            type: `GG`,
            data: { stacks: d, source: this.source, tileID: this.tileID, type: `glyphs` },
          },
          e,
        )));
    }
    let h = Object.keys(l.iconDependencies),
      g = Promise.resolve({});
    if (h.length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
        (g = i.sendAsync(
          {
            type: `GI`,
            data: { icons: h, source: this.source, tileID: this.tileID, type: `icons` },
          },
          e,
        )));
    }
    ...
    let w = l.dashDependencies,
      T = Promise.resolve({});
    if (Object.keys(w).length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
        (T = i.sendAsync({ type: `GDA`, data: { dashes: w } }, e)));
    }
    let [E, D, O, k] = await Promise.all([p, g, C, T]),
      A = new G(E),
      j = new f(D, O);
    for (let e in c) {
      let t = c[e];
      t instanceof r
        ? (q(t.layers, this.zoom, n),
          te({
            bucket: t,
            glyphMap: E,
            glyphPositions: A.positions,
            imageMap: D,
            imagePositions: j.iconPositions,
            showCollisionBoxes: this.showCollisionBoxes,
            canonical: this.tileID.canonical,
            subdivisionGranularity: l.subdivisionGranularity,
          }))
        : t.hasDependencies &&
          (t instanceof ee || t instanceof N || t instanceof v) &&
          (q(t.layers, this.zoom, n),
          t.addFeatures(l, this.tileID.canonical, j.patternPositions, k));
    }
    return {
      buckets: Object.values(c).filter((e) => !e.isEmpty()),
      featureIndex: s,
      collisionBoxArray: this.collisionBoxArray,
      glyphAtlasImage: A.image,
      imageAtlas: j,
      dashPositions: k,
      glyphMap: this.returnDependencies ? E : null,
      iconMap: this.returnDependencies ? D : null,
      glyphPositions: this.returnDependencies ? A.positions : null,
    };
  }
};
function q(e, t, n) {
...
var J = class {
  ...
  ae = class {
    ...
    loadGeometry() {
      return this.pointsArray.map((e) => e.map((e) => new n(e.x, e.y)));
    }
  },
  oe = class {
    constructor(e, t, n) {
...
function ce(e, t, n) {
  ...
  for (let t = 0; t < e.length; t++) {
    ...
    ((c = E(c, n.type, -128, -128, r + 128, r + 128)),
      c.length !== 0 && s.push(new ae(n.type, c, n.properties, n.id, r)));
  }
  return new oe(s, e.name, r);
}
var le = class {
    constructor(e, t, n) {
      ((this.actor = e),
    ...
    loadVectorTile(e, n) {
      try {
        return {
          vectorTile: e.encoding === `mlt` ? new k(n) : new w(new t(n)),
          rawData: n,
        };
      } catch (t) {
        let r = new Uint8Array(n),
          i = r[0] === 31 && r[1] === 139,
          a = `Unable to parse the tile at ${e.request.url}, `;
        throw (
          (a += i
            ? `please make sure the data is not gzipped and that you have configured the relevant header in the server`
            : `got error: ${F(t).message}`),
          Error(a)
        );
      }
    }
    async loadTile(e) {
      ...
      i.abort = o;
      try {
        let s = await a(e.request, o);
        if (e.etag && e.etag === s.etag)
          return (this.tileState.finishLoading(t), this._getEtagUnmodifiedResult(s, r));
        let c = this.loadVectorTile(e, s.data);
        if ((this.tileState.finishLoading(t), !c)) return null;
        let { vectorTile: l, rawData: u } = c;
        n && ({ vectorTile: l, rawData: u } = this._getOverzoomTile(e, l));
        let d = this._getExpiryData(s),
          f = this._finishRequestTiming(r);
        ((i.vectorTile = l), (i.etag = s.etag), this.tileState.markLoaded(t, i));
        let p = { rawData: u, cacheControl: d, resourceTiming: f };
        return (this.tileState.setParsing(t, p), await this._parseWorkerTile(i, e));
      } catch (e) {
        throw (this.tileState.finishLoading(t), this.tileState.markLoaded(t, i), e);
      }
    }
    _getEtagUnmodifiedResult(e, t) {
      let n = this._getExpiryData(e),
        r = this._finishRequestTiming(t);
      return y({ etagUnmodified: !0 }, n, r);
    }
    async _parseWorkerTile(e, t) {
      ...
      if (n) {
        ...
        ((r = y({ rawTileData: i.slice(0), encoding: s }, r, a, o)),
          this.tileState.removeParsing(e.uid));
      } else e.etag && (r = y(r, { etag: e.etag }));
      return r;
    }
    ...
    _startRequestTiming(e) {
      if (e.request?.collectResourceTiming) return new Y(e.request.url);
    }
    ...
    _getOverzoomTile(e, t) {
      ...
      for (let e in l) {
        let r = t.layers[e];
        if (!r) continue;
        let i = ce(r, a, n.canonical);
        i.length > 0 && c.addLayer(i);
      }
      let u = { vectorTile: c, rawData: A(c).buffer };
  ...
  X = class {
    ...
    async loadTile(e) {
      let {
        ...
        c = r.width + 2,
        l = r.height + 2,
        u = M(r) ? new O({ width: c, height: l }, await C(r, -1, -1, c, l)) : r,
        d = new g(t, u, n, i, a, o, s);
      return ((this.loaded ||= {}), (this.loaded[t] = d), d);
    }
    removeTile(e) {
      let t = this.loaded,
        n = e.uid;
  ...
  ue = class {
    ...
    loadVectorTile(e) {
      ...
      if (!i) return null;
      let a = new re(i.features, { version: 2, extent: s });
      return { vectorTile: a, rawData: A(a, B).buffer };
    }
    async loadTile(e) {
      let { uid: t } = e,
        n = new K(e);
      ...
      try {
        ...
        let o = { rawData: a };
        return (this.tileState.setParsing(t, o), await this._parseWorkerTile(n, e));
      } catch (e) {
    ...
    async _parseWorkerTile(e, t) {
      ...
      if (n) {
        let { rawData: t } = n;
        ((r = y({ rawTileData: t.slice(0), encoding: `mvt` }, r)),
          this.tileState.removeParsing(e.uid));
      }
      return r;
    }
    ...
    async loadData(e) {
      ...
      try {
        ...
        let n = {};
        return (e.request && (n.data = e.data), this._finishRequestTiming(t, e, n), n);
      } catch (e) {
        if ((delete this._pendingRequest, !l(e))) throw e;
        return { abandoned: !0 };
      }
    }
    _startRequestTiming(e) {
    ...
    async loadAndProcessGeoJSON(e, t) {
      if ((e.request && (e.data = (await i(e.request, t)).data), e.data)) {
      ...
      if (
        (e.updateCluster &&
          this._geoJSONIndex.updateClusterOptions(e.geojsonVtOptions.cluster, Z(e)),
        this._geoJSONIndex == null)
      )
        throw Error(`Input data given to '${e.source}' is not a valid GeoJSON object.`);
    }
...
function de(e, t) {
  let n = y(t.geojsonVtOptions || {}, { updateable: !0, clusterOptions: Z(t) });
  return new o(e, n);
}
function Z({ geojsonVtOptions: e, clusterProperties: t, source: n }) {
  ...
  for (let e of s) {
    let [a, o] = t[e],
      s = j(o, `sources.${n}.clusterProperties.${e}[1]`),
      c = j(
...
var $ = class {
  constructor(t) {
    ((this.self = t),
      ...
      this.actor.registerMessageHandler(`SRPS`, (e, t) => this._syncRTLPluginState(e, t)),
  ...
  _getLayerIndex(e) {
    ...
    return ((t ||= this.layerIndexes[e] = new W()), t);
  }
  _getWorkerSource(e, t, n) {
    if (
      ...
      switch (t) {
        case `vector`:
          this.workerSources[e][t][n] = new le(
            r,
          ...
          break;
        case `geojson`:
          this.workerSources[e][t][n] = new ue(
            r,
            this._getLayerIndex(e),
            this._getAvailableImages(e),
          );
  ...
  _getDEMWorkerSource(e, t) {
    return (
      ...
      (this.demWorkerSources[e][t] ||= new X()),
